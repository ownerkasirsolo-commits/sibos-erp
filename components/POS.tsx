...
import { usePOSLogic } from '../hooks/usePOSLogic'; // Import Hook

// Atomic Components
import POSHeader from './pos/POSHeader';
import POSProductGrid from './pos/POSProductGrid';
import POSCart from './pos/POSCart';
import POSActionPad from './pos/POSActionPad';
...
const POS: React.FC<POSProps> = ({ businessType, onExit }) => {
  ...
  const {
    ...
    // Actions
    handleNumpadInput, toggleOrderType, handleClearCart, handleAcceptDelivery,
  ...
  } = usePOSLogic(businessType);

  return (
    <div className="flex h-screen bg-[#0f172a] overflow-hidden">
      
      {/* 50% LEFT: PRODUCT CATALOG */}
      ...
      {/* SHIFT MODAL - Auto Assign User */}
      <Modal isOpen={(!currentShift || currentShift.status === 'closed') && isShiftModalOpen} onClose={() => {}} title="Buka Shift">
           ...
           <div className="space-y-4 text-left mt-6">
               <GlassPanel className="p-4 rounded-xl border border-white/10">
                   <label className="text-xs font-bold text-gray-500 uppercase">Kasir</label>
                   <p className="text-lg font-bold text-white mt-1">{currentUser?.name}</p>
                   <p className="text-xs text-gray-400">{currentUser?.role}</p>
               </GlassPanel>

               <div>
                   <label className="text-xs font-bold text-gray-500 uppercase">Modal Awal</label>
      ...
      {/* HISTORY MODAL */}
      <Modal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} title="Riwayat" size="lg">
           <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
               {transactions.length === 0 ? (
                   <div className="text-center text-gray-500 py-10">Kosong</div>
               ) : (
