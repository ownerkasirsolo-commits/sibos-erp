...
import { usePOReceiveLogic } from '../../../hooks/useProcurementLogic';
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <div className="glass-panel w-full max-w-2xl p-6 rounded-3xl relative z-10 animate-in zoom-in-95 flex flex-col max-h-[90vh]">
                ...
                <div className="flex gap-3 mt-6 pt-4 border-t border-white/10">
                    {receiveWizardStep === 1 ? (
                        <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-white/10 text-gray-400">Batal</button>
                    ) : (
                        <button onClick={() => setReceiveWizardStep(prev => prev - 1 as any)} className="flex-1 py-3 rounded-xl border border-white/10 text-gray-400">Kembali</button>
                    )}
                    
                    {receiveWizardStep < 3 ? (
                        <button onClick={() => setReceiveWizardStep(prev => prev + 1 as any)} className="flex-1 py-3 bg-orange-600 hover:bg-orange-500 text-white font-bold rounded-xl">Lanjut</button>
                    ) : (
                        <button onClick={handleSubmitReceive} className="flex-1 py-3 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl flex items-center justify-center gap-2">
