...
import { Table, PauseCircle, History, DollarSign, Disc, Split, StickyNote, Trash2, FileText, CreditCard, QrCode, ArrowRight, Delete } from 'lucide-react';
// @FIX: Employee type moved to its own feature module.
import { Shift } from '../../types';
import { Employee } from '../../features/hrm/types';
...
const POSActionPad: React.FC<POSActionPadProps> = ({ 
  total, cashReceived, change, handleNumpadInput, currentShift, currentUser,
    ...
    <div className="w-[27%] flex flex-col bg-[#111827] border-l border-white/5 h-full">
      ...
      <div className="shrink-0 bg-black/40 flex flex-col justify-center p-4 border-b border-white/5 relative gap-2">
          <div className="flex justify-between items-baseline">
              <div className="flex items-center gap-2">
                  ...
                  {orderType === 'take-away' && <span className="text-[10px] bg-blue-500 text-white px-1.5 rounded font-bold">TAKEAWAY</span>}
              </div>
          ...
          <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/10">
              <div className="text-right">
                  ...
                  <span className={`text-xl font-bold ${cashReceived < total && cashReceived > 0 ? 'text-red-400' : 'text-gray-300'}`}>
           ...
           {/* Staff Info */}
           <div className="absolute bottom-1 left-2 text-[10px] text-gray-600 font-mono">
               {currentShift ? `Shift: ${currentShift.staffName}` : `Logged: ${currentUser?.name}`}
           </div>
      </div>
      ...
      <div className="flex-1 p-2 grid grid-cols-4 grid-rows-7 gap-1 h-full">
          ...
          <button 
            ...
            className={`col-span-1 bg-orange-500/20 hover:bg-orange-500 text-white border border-orange-500/30 rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 h-full transition-all ${pendingCount > 0 && !hasCart ? 'animate-pulse border-orange-400' : ''}`}
          >
              <PauseCircle size={16} className="text-white" /> <span className="text-[9px]">{hasCart ? 'Pending' : `List (${pendingCount})`}</span>
          </button>

          <button onClick={onHistory} className="col-span-1 bg-orange-500/20 hover:bg-orange-500 text-white border border-orange-500/30 rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 h-full transition-all">
              <History size={16} className="text-white" /> <span className="text-[9px]">Riwayat</span>
          </button>
          <button onClick={onDrawer} className="col-span-1 bg-orange-500/20 hover:bg-orange-500 text-white border border-orange-500/30 rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 h-full transition-all">
              <DollarSign size={16} className="text-white" /> <span className="text-[9px]">Kas</span>
          </button>

           <button onClick={() => alert("Disc Global")} className="col-span-1 bg-orange-500/20 hover:bg-orange-500 text-white border border-orange-500/30 rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 h-full transition-all">
              <Disc size={16} className="text-white" /> <span className="text-[9px]">Diskon</span>
          </button>
          <button onClick={onSplit} className="col-span-1 bg-orange-500/20 hover:bg-orange-500 text-white border border-orange-500/30 rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 h-full transition-all">
              <Split size={16} className="text-white" /> <span className="text-[9px]">Split</span>
          </button>
          <button onClick={() => alert("Fitur Catatan Global")} className="col-span-1 bg-orange-500/20 hover:bg-orange-500 text-white border border-orange-500/30 rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 h-full transition-all">
              <StickyNote size={16} className="text-white" /> <span className="text-[9px]">Catatan</span>
          </button>
          ...
          <button onClick={() => onCheckout('debt')} className="col-span-1 bg-purple-600/20 hover:bg-purple-600 text-white rounded-lg font-bold text-xs flex flex-col items-center justify-center gap-0.5 border border-purple-600/30 h-full">
          ...
          {[1,2,3].map(n => <button key={n} onClick={() => handleNumpadInput(n.toString())} className="bg-gray-800 hover:bg-gray-700 text-white font-bold text-xl rounded-lg h-full">{n}</button>)}
           ...
           <button 
            ...
            className="row-span-2 bg-gradient-to-br from-orange-600 to-red-600 hover:brightness-110 text-white font-bold rounded-lg shadow-lg flex flex-col items-center justify-center gap-1 active:scale-95 transition-transform h-full"
          >
