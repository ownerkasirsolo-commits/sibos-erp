...
const Auth: React.FC = () => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedEmployeeId && employees.length > 0) {
        ...
        await setBusinessInfo("SIBOS Resto & Cafe", BusinessType.CULINARY);
        await login(boss.id, boss.pin || '0000');
        return;
    }
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    await setBusinessInfo(businessName || "Bisnis Baru", selectedType);
    // Auto login as Manager (first mock employee for demo purposes)
    if(employees.length > 0) await login(employees[0].id, employees[0].pin || '1234');
  };
