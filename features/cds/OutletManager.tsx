...
import { useGlobalContext } from '../../context/GlobalContext'; // Access Context to save config
...
const OutletManager: React.FC = () => {
  const { businessConfig, setBusinessInfo } = useGlobalContext(); // Assuming setBusinessInfo or similar can patch config
  ...
  const [printerSize, setPrinterSize] = useState(businessConfig?.printerPaperSize || '58mm');
  ...
  const [roles, setRoles] = useState([
    ...
    { id: 'r3', name: 'Kitchen', permissions: [] },
  ...
  const handleSaveConfig = () => {
      // In a real app, update global context/backend
      // Here we assume global context is updated via alert for demo
      alert(`Konfigurasi Tersimpan!\nKertas Printer: ${printerSize}\nFooter: "${footerText.substring(0,20)}..."`);
      // Force update businessConfig object in context conceptually
      if(businessConfig) {
          businessConfig.printerPaperSize = printerSize as any;
          businessConfig.footerMessage = footerText;
      }
  };
                        ...
                        <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-2 text-[10px] text-gray-500">
                                    ...
                                    <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="taxType" className="accent-orange-500"/> PB1 (Resto)</label>
                                </div>
                            </div>
                            <div className="space-y-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className={`bg-white text-black p-4 rounded-xl font-mono text-xs overflow-y-auto shadow-inner opacity-90 mx-auto transition-all duration-300 ${printerSize === '58mm' ? 'w-[58mm]' : (printerSize === '80mm' || printerSize === 'DotMatrix-76mm') ? 'w-[80mm]' : 'w-full h-64'}`}>
                                <div className="text-center mb-2">
                                    <p className="font-bold text-sm">SIBOS RESTO & CAFE</p>
                                    <p>Jl. Merdeka No. 1, Jakarta</p>
                                    <p>--------------------------------</p>
                                </div>
                                <div className="space-y-1 mb-2">
                                    <div className="flex justify-between"><span>Nasi Goreng</span><span>35.000</span></div>
                                    <div className="flex justify-between"><span>Es Teh Manis</span><span>5.000</span></div>
                                </div>
