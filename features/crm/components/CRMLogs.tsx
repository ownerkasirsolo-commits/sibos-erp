...
import { Activity, Search, History, UserCheck, ShoppingBag, Database, Star } from 'lucide-react';
...
const CRMLogs: React.FC = () => {
    const { liveLogs } = useCRMLogic();
    const [searchTerm, setSearchTerm] = useState('');
    const [activeCategory, setActiveCategory] = useState('All');

    // Filter Logic
    const visibleLogs = liveLogs.filter(log => {
    ...
    const categories = ['All', 'Database', 'Transaction', 'Loyalty', 'System'];

    const getIcon = (type: string) => {
        if (type === 'Transaction') return <ShoppingBag size={14} />;
        if (type === 'Database') return <Database size={14} />;
        if (type === 'Loyalty') return <Star size={14} />;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in slide-in-from-right-4 relative">
            ...
            <div className="lg:col-span-9 flex flex-col gap-4">
                ...
                <div className="bg-white/5 p-4 rounded-2xl border border-white/5 space-y-4">
                    <SectionHeader 
                        ...
                        subtitle="Jejak interaksi, pendaftaran, dan transaksi member."
                    />
                    
                    <div className="mt-4">
                        <GlassInput 
                            icon={Search} 
                            placeholder="Cari aktivitas, nama member..." 
                            value={searchTerm}
            ...
            <div className="lg:col-span-3 flex flex-col gap-4">
                <GlassPanel className="p-5 rounded-2xl border border-white/5">
                    ...
                    <div className="space-y-2">
                        {categories.map(cat => (
                            ...
                            >
                                <span>{cat}</span>
                                {cat !== 'All' && <span className="bg-black/20 px-1.5 rounded text-[9px] opacity-70">
                                    {liveLogs.filter(l => l.category === cat).length}
                                </span>}
                            </button>
                        ))}
                    </div>
                ...
                <div className="p-4 rounded-xl bg-blue-500/5 border border-blue-500/10 text-xs text-blue-200/80 leading-relaxed">
                    ...
                    Log aktivitas pelanggan disimpan selama 12 bulan untuk keperluan analisis retensi dan audit poin.
                </div>
            </div>
        </div>
    );
