...
import { useKDSLogic } from './hooks/useKDSLogic';
          ...
          {activeOrders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {activeOrders.map(order => {
                    // Filter items: Only show CULINARY items in KDS card
                    const kitchenItems = order.items.filter(i => i.businessType === BusinessType.CULINARY || i.hasRecipe);
                    
                    if (kitchenItems.length === 0) return null; // Should not happen due to hook logic, but safe guard
                    ...
                    <div key={order.id} className={`bg-[#121a2c] border rounded-2xl overflow-hidden flex flex-col transition-all duration-300 ${getStatusGlow(order.status)} ${['paid', 'debt'].includes(order.status) ? 'border-red-500/30' : 'border-white/10'}`}>
                        ...
                        {kitchenItems.map((item, idx) => (
                            ...
                            <div>
                                ...
                                {/* Optional: Show Modifiers if any */}
                                {item.selectedModifiers && item.selectedModifiers.length > 0 && (
                                    <div className="text-[10px] text-gray-500 mt-1 pl-1 border-l-2 border-white/10">
