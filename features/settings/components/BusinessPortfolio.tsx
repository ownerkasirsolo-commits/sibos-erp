...
const BusinessPortfolio: React.FC = () => {
  ...
  const handleBusinessClick = (bizId: string) => {
    if (expandedBusinessId === bizId && selectedOutletId === null) {
      // If clicking the already expanded and selected business, collapse it
      setExpandedBusinessId(null);
    } else {
      // Otherwise, expand this business and show its main settings
      setExpandedBusinessId(bizId);
      setSelectedOutletId(null); // Deselect any outlet
      ...
      `}>
        ...
        <div className="space-y-3">
            {MOCK_BUSINESSES.map((biz) => {
                ...
                const isBusinessActiveForEdit = isExpanded && selectedOutletId === null;
                ...
                return (
                    <div key={biz.id} className={`space-y-2 ${!biz.active ? 'opacity-60' : ''}`}>
                        ...
                        <div 
                            ...
                            className={`glass-panel p-4 rounded-2xl cursor-pointer transition-all duration-200 border relative overflow-hidden 
                                ${isBusinessActiveForEdit 
                                    ? 'border-orange-500 shadow-[0_0_20px_-10px_rgba(249,115,22,0.5)] bg-orange-500/5' 
                                    : isExpanded
                                        ? 'border-white/10 bg-white/5'
                                        : 'border-transparent hover:border-white/10 hover:bg-white/5'
                                }`}
                        >
                            {isBusinessActiveForEdit && <div className="absolute left-0 top-0 bottom-0 w-1 bg-orange-500"></div>}
                            
                            <div className="flex items-center gap-4">
                                <img src={biz.logo} alt={biz.name} className="w-12 h-12 rounded-xl object-cover bg-gray-800" />
                                <div className="flex-1 min-w-0">
                                    <h4 className={`font-bold text-sm truncate ${isExpanded ? 'text-white' : 'text-gray-300'}`}>{biz.name}</h4>
